...
class PointMLPEncoder(nn.Module):
    """
    Encoder portion of PointMLP: converts a point cloud [B, 3, N] into a global feature [B, D].
    """
    ...
    def __init__(
        ...
        # Create lists of modules for each stage
        self.local_grouper_list = nn.ModuleList()
        self.pre_blocks_list = nn.ModuleList()
        self.pos_blocks_list = nn.ModuleList()
        ...
        for i in range(self.stages):
            ...
            # Pre-extraction MLPs over each local patch
            ...
            # Positional MLPs along anchor points
    ...
    def forward(self, x, return_sequence=False):
        ...
        # Apply each hierarchical stage
        for i in range(self.stages):
            xyz, x = self.local_grouper_list[i](xyz, x.permute(0, 2, 1))  # xyz:[B,S,3], grouped:[B,S,K,D]
            x = self.pre_blocks_list[i](x)  # [B, D_out, S]
            x = self.pos_blocks_list[i](x)  # [B, D_out, S]

        if return_sequence:
            return x  # [B, D_last, S_last]

            # otherwise do global pooling as before
        x = F.adaptive_max_pool1d(x, 1).squeeze(-1)  # [B, D_last]
        return x
